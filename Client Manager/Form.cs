...
namespace Client_Manager
{
    public partial class Form1 : Form
    {
        ...
        private void modifybtn_Click(object sender, EventArgs e)
        {
            ...
            String query = "Update Clients Set FName = @p1, LName = @p2, Adress = @p3, City = @p4 Where ID =@p5 ";
            using(SqlConnection con = new SqlConnection(conString))
            {
                position = this.dgvclients.CurrentRow.Index;
                id = int.Parse(this.dgvclients.Rows[position].Cells[0].Value.ToString());
                ...
                SqlCommand cmd = new SqlCommand(query, con);
                cmd.Parameters.AddWithValue("@p1", this.fninput.Text.Trim().ToUpper());
                cmd.Parameters.AddWithValue("@p2", this.lninput.Text.Trim().ToUpper());
                cmd.Parameters.AddWithValue("@p3", this.addressinput.Text.Trim().ToLower());
                cmd.Parameters.AddWithValue("@p4", this.cityinput.Text.Trim().ToUpper());
        ...
        {
            check = false;
            this.fninput.Text = string.Empty;
            this.lninput.Text = string.Empty;
            this.addressinput.Text = string.Empty;
